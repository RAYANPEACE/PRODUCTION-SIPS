...
- **Phase 7 (inventaire fragmente via serveur, base commune + freshCodes)** : PARTIELLEMENT FAIT (cache SW v103). Sessions serveur, pre-comptage hors serveur, chargement de base commune, contributions strictes, detection de conflits et resolution explicite admin sont codees. A tester sur mobile et a raffiner UX si besoin.
