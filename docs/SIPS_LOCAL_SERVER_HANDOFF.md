...
- **2026-07-02 - fiche validee lecture seule stricte + anti-reentrance decision** (sur `main`, SW v205) : (1) "Voir" sur record VALIDEE serveur (prod + mouvements) route via sipsViewValidatedProd/Mov -> vue lecture seule (flag PFVIEW/MOVVIEW.ro), brouillon stashe/restaure, aucun Save/Submit/Valider/Rejeter (avant : ecrasait le brouillon + record editable/resoumissible) ; (2) garde id-scoped en tete de sipsDecide (une decision en vol par soumission, toutes surfaces). Passe au gate adversarial Codex (base 14b2a37) : finding serveur "doublon record" rejete = deja serialise via withDbLock (app.mjs:925) + 409 (app.mjs:1620). `check:js` OK. **A tester mobile.**
- **2026-07-02 - Voir la fiche par onglet + prod compacte + fix lot** (sur `main`, SW v203) : (1) fix numero de lot qualite (inclut serveur, evite LOT-001 bloque) ; (2) "Voir" deplace du Serveur vers Prod/Entree/Sortie (comme Qualite) : liste des soumissions en attente + Voir -> formulaire reel en LECTURE SEULE (brouillon stashe/restaure, Valider/Rejeter/Retour via sipsDecide), retrait vue plein ecran serveur ; (3) mini historique production compact (1 ligne/produit). `check:js` OK. **A tester mobile + gate Codex demande.**
