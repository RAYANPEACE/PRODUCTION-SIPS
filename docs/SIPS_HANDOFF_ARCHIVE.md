...
- 2026-06-24 : **Spec C — inventaire fragmente (multi-compteurs, hors-ligne d'abord) implementee.** (branche `inventaire-serveur`, plan `docs/superpowers/plans/2026-06-24-inventaire-fragmente.md`, gate Codex toujours du avant merge `main`). On **inverse** le modele Phase 7 (plus de session live a rejoindre). **Serveur** (`server/app.mjs`, commit `c9aff47`) : (1) **manche unique** `findOrCreateOpenRound` (au plus une session ouverte) + `POST /api/inventory-rounds/contribution` **sans sessionId** (rattache a la manche ouverte, la cree si aucune, base = dernier inventaire valide) ; (2) **assemblage revise** (`buildInventoryPayloadFromSession`) : un article non compte par PERSONNE = `{counted:false}` (ecart nul, **jamais** la valeur de base) ; chaque article compte est estampille `by` (nom) + `byUser` (identite stable) ; conflit (>1 compteur) -> 409 sans resolution (conserve) ; (3) **recompte cible** : `reject` inventaire accepte `recountArticles[{code,by,byUser}]` ; `GET /api/submissions?status=rejected&type=inventory&forMe=1` -> un compteur liste les soumissions rejetees dont un article LUI est attribue (`byUser`), filtre sur ses seuls articles, sans payload d'autrui ; `publicSubmission` expose `recountArticles`. Tests `[C1]`-`[C5]` (12 checks), `test:server` = **47/47**. **Client** : (4) part **hors-ligne d'abord** — `srvFragSubmitMine` envoie vers la manche ouverte ; hors-ligne -> file `lep_server_pending` (type `frag-contribution`), `sipsFlushPending` draine vers le endpoint sans id (auto-envoi a la reconnexion) — SW v125 ; (5) **assemblage** : `srvRoundCoverage` (couverture comptes/par-qui / non comptes / conflits) + `srvFragAnalyze` enchaine sur le **Bilan de revue B** apres finalize — SW v126 ; (6) **recompte cible** : le Bilan de revue propose de renvoyer les articles anormaux a LEUR compteur (`recountArticles`), section **« A recompter (articles cibles) »** cote compteur via `forMe`, **« compte par X »** affiche sur chaque ligne du Bilan — SW v127 ; (7) **retrait du secours fichier fragmente** (export/import/fusion `.txt`, bloc `#fragModeFiles`), le flux serveur offline-first le remplace ; conserve le mode « meme telephone » + Sauvegarde complete — SW **v128**. `check:js` OK partout. **A TESTER SUR MOBILE** : 2 telephones comptent des zones differentes hors-ligne -> admin rallume -> assemblage (couverture + conflit) -> comparer (Bilan) -> recompte cible d'un article -> le bon compteur le recoit -> re-comparer -> valider -> consultation montre « compte par X ». Surface sensible (donnees inventaire multi-utilisateurs, reconstruction serveur, isolation recompte cible) : **gate cross-model Codex conseillee avant merge `main`** (voir memoire [[project_sips-pending-reviews]]).
